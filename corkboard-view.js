const corkboardFilenameBase = 'corkboard';
const isMac = false;
...
function showCorkboard(project){
    ...
    popup.classList.add("popup-corkboard");

    var corkboard = document.createElement('div');
    corkboard.id = 'corkboard';
    popup.appendChild(corkboard);

    document.getElementById('corkboard-container').appendChild(popup);

    loadedCards = parseCardsString(document.getElementById('outline').value);
    if(!loadedCards)
...
function getTitleBar(){
  ...
  corkboardTitle.innerText = 'Markdown Corkboard';
...
function createCardSpot(num, owningCol, posInCol) {
  ...
  card.addEventListener('keyup', cardCntrlEventsKeyUp);
...
function uploadTextFile(){
  ...
  fileInput.addEventListener('change', function(e){
    ...
    reader.onload = () => {
      document.getElementById('outline').value = reader.result;
      showCorkboard(project);
...
function getTitleFromFilename(filename){
  var title = '';
  const titleRegx = new RegExp('(.*)_' + corkboardFilenameBase + '.*');
  const titleMatch = titleRegx.exec(filename);
  if(titleMatch)
    title = titleMatch[1];
  return title;
}
...
function cardCntrlEventsKeyUp(e){
    var outlineString = generateCardsString(loadedCards);
    document.getElementById('outline').value = outlineString;
}
...
function saveCards(cards){
    ...
    filename += corkboardFilenameBase + '_' + getPaddedDateTimeString() + '.txt';
